...
open class PieChartRenderer: DataRenderer
{
    ...
    open func drawDataSet(context: CGContext, dataSet: IPieChartDataSet)
    {
        ...
        pathInner.addArc(center: center, radius: centerLineRadius, startAngle: CGFloat(-M_PI_2), endAngle: CGFloat(M_PI_2*3), clockwise: false)
        ...
        for j in 0 ..< entryCount
        {
            ...
            if (abs(e.y) > DBL_EPSILON)
            {
                if !chart.needsHighlight(index: j)
                {
                    ...
                    path.addArc(center: CGPoint(x: х1, y: y1), radius: linePointRadius, startAngle: CGFloat(-M_PI_2), endAngle: CGFloat(M_PI_2*3), clockwise: false)
                    path.closeSubpath()
    
                    path.move(to: CGPoint(x: x2, y: y2))
                    path.addArc(center: CGPoint(x: x2, y: y2), radius: linePointRadius, startAngle: CGFloat(-M_PI_2), endAngle: CGFloat(M_PI_2*3), clockwise: false)
                    path.closeSubpath()
